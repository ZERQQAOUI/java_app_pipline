pipeline {
    ...
    environment {
        ...
        DOCKER_HOSTED_EP = "13.235.51.27:8083"
        HELM_HOSTED_EP = "13.235.51.27:8081"
    }
    stages{
        ...
        stage("Deployment Approval"){
            steps{
                script{
                    timeout(10){
                          mail bcc: '', body: "<br>Project: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> Goto : ${env.BUILD_URL} and approve/reject the deployment", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "CICD APPROVAL REQUEST: Project name -> ${env.JOB_NAME}", to: "zerqqaouiyassine1@gmail.com";  
